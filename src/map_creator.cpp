...
//clock_t startTime, stopTime, diffTime, countTime=1;
//double aveTime;
void ExpandMap::expandObstacle(const nav_msgs::OccupancyGrid& map_in){
		...
		for(int yi=0; yi<(int)map_in.info.width; yi++){
			...
			if(map_in.data[tmp_num]!=FREE){
				...
				for(litr=expanded_circle.begin(); litr!=expanded_circle.end(); litr++){
					...
					tmp_num_ = x + map_in.info.width*y;
					if(x>=0 && x<(int)local_map.info.height && 	y>=0 && y<(int)local_map.info.width
						...
						&& map_in.data[tmp_num]>local_map.data[tmp_num_]){
						//local_map.data[x+map_in.info.width*y]=map_in.data[xi+map_in.info.width*yi];
						local_map.data[tmp_num_]=map_in.data[tmp_num];
					}
				}
			}
		}
	...
	//aveTime = (double)( aveTime*(countTime-1)+diffTime ) / (double)countTime;
	////printf("%ld\n", diffTime);
	//printf("%ld, %lf\n", diffTime, aveTime);
	//countTime += 1;
}
...
void pcCallback(const sensor_msgs::PointCloudConstPtr& msg)
{
	...
	for(i=0; i<msg->points.size(); i++){
		if(0.3<msg->points[i].z && msg->points[i].z < 0.6){
...
int main(int argc, char** argv)
{
	...
	while(ros::ok()){
		...
		//diffTime = stopTime - startTime;
		//aveTime = (double)( aveTime*(countTime-1)+diffTime ) / (double)countTime;
		//printf("It takes %ld [ms], ave: %lf [ms]\n", diffTime, aveTime);
		//countTime += 1;
